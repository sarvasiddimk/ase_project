...
async function bootstrap() {
    ...
    // Create Service Jobs
    const job1 = await serviceJobsService.create({
        customerId: customer1.id,
        vehicleId: vehicle1.id,
        description: 'Oil Change & Brake Inspection'
    });

    const job2 = await serviceJobsService.create({
        customerId: customer2.id,
        vehicleId: vehicle2.id,
        description: 'Check Engine Light Diagnosis'
    });

    const job3 = await serviceJobsService.create({
        customerId: customer3.id,
    ...
    await schedulingService.create({
        ...
        endTime: new Date(appointment2Time.getTime() + 120 * 60 * 1000).toISOString(),
        notes: 'Brake Service'
    });
    ...
    await schedulingService.create({
        ...
        endTime: new Date(appointment3Time.getTime() + 60 * 60 * 1000).toISOString(),
        notes: 'Tire Rotation'
    });
