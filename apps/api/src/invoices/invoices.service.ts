...
export class InvoicesService {
    ...
    async create(createInvoiceDto: CreateInvoiceDto): Promise<Invoice> {
        ...
        if (!job) {
            throw new NotFoundException(`Job with ID ${createInvoiceDto.jobId} not found`);
        }

        if (job.invoiceId) {
            throw new BadRequestException(`Job ${createInvoiceDto.jobId} already has an invoice`);
        }

        // Calculate totals
        const subtotal = job.items.reduce((sum, item) => sum + Number(item.totalPrice), 0);
        const taxRate = 0.10; // 10% tax for example
